...
def initialize_model(config, train_source, test_source):
    ...
    model_param = deepcopy(model_config)#复制
    model_param['train_source'] = train_source
    model_param['test_source'] = test_source
    model_param['train_pid_num'] = data_config['pid_num']#73
    batch_size = int(np.prod(model_config['batch_size']))#'batch_size': (8, 16) batch_size = 8*16 =
    model_param['save_name'] = '_'.join(map(str,[
        model_config['model_name'],#'GaitSet'
        data_config['dataset'],#'CASIA-B'
        data_config['pid_num'],#73
        data_config['pid_shuffle'],#Flase
        model_config['hidden_dim'],#256
        model_config['margin'],#0.2
        batch_size,#8*16
        model_config['hard_or_full_trip'],#'full'
        model_config['frame_num'],#30
    ]))
...
def initialization(config, train=False, test=False):
    ...
    os.chdir(WORK_PATH)#改变当前工作路径至work
